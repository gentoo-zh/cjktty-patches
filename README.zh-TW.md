...
- 記錄目前這批點版本，全部沿用原有補丁。只有 6.1.188 動到補丁觸及的檔案：兩個 `vt.c` 寫入輔助函式的參數由 `unsigned char` 改為 `u8`，落在所有 cjktty hunk 之外，因此 `cjktty-6.1.184.patch` 仍可套用。
...
- 記錄 2026-09-02 這批八個點版本，全部沿用原有補丁；上游未觸及 cjktty 修改的任何檔案。
- 新增 Linux 7.3-rc1 的補丁。7.2 的補丁套到 7.3 合併視窗只有一個 hunk 位移、沒有 reject，因此新檔案帶的是同一組改動。
- Linux 7.1 於 2026-09-02 終止維護，離開支援矩陣；其最後一版仍然套用、建置與算繪正常。
- `tools/regen.sh` 改為把 `-0,0` 的 hunk 也視為新檔標記。本倉庫的補丁不寫 `--- /dev/null`，因此移植過去無法重新產生補丁。
