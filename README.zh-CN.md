...
补丁基于 [gentoo-zh/linux-cjktty](https://github.com/gentoo-zh/linux-cjktty)；本仓库追加的修复记录在 [CHANGELOG.md](CHANGELOG.md)。
...
- 记录当前这批点版本，全部沿用原有补丁。只有 6.1.188 动到补丁触及的文件：两个 `vt.c` 写入辅助函数的形参由 `unsigned char` 改为 `u8`，落在所有 cjktty hunk 之外，因此 `cjktty-6.1.184.patch` 仍可套用。

### 2026.9.9 / 5.15.220, 6.1.187, 6.6.156, 6.12.109, 6.18.50, 7.2.4, 7.3-rc2

- 改由 `vc_uni_lines` 取得码点，不再靠指针越过屏幕缓冲区去读私有平面。单一来源不会和自己失步，因此五个同步点与 `con_putc()` 那个双字绕法一并消失。
- fallback 路径上每个字形索引都以 `vc_font.charcount` 设限；提供者依字体几何选取，不再依存储格字节大小；只有真正存有 fallback 码点的位置才把字形槽 254 与 255 视为续接标记。
- 先问已加载的字体，字体自己映射了 U+00FF 以上的码点就用它的字形。
- 未应用 `cjktty-add-cjk32x32-font-data.patch` 就设置 `CONFIG_FONT_CJK_32x32` 时构建直接失败。以往它会编出一份 descriptor 声称有字形、数据却是空的字体。`CONFIG_FONT_CJK_16x16` 不再默认开启。
- Linux 5.10 沿用原补丁。它的 `struct fbcon_ops` 渲染器与 `struct fbcon_par` 相距过远，无法机械移植，且该系列接近终止维护。

### 2026.9.3 / 5.10.269, 5.15.220, 6.1.187, 6.6.156, 6.12.108, 6.18.49, 7.1.13, 7.2.3, 7.3-rc1

- 记录 2026-09-02 这批八个点版本，全部沿用原有补丁；上游未触及 cjktty 修改的任何文件。
- 新增 Linux 7.3-rc1 的补丁。7.2 的补丁套到 7.3 合并窗口只有一个 hunk 偏移、没有 reject，因此新文件带的是同一组改动。
- Linux 7.1 于 2026-09-02 终止维护，离开支持矩阵；其最后一版仍然应用、构建与渲染正常。
- `tools/regen.sh` 改为把 `-0,0` 的 hunk 也视为新文件标记。本仓库的补丁不写 `--- /dev/null`，因此移植过去无法重新生成补丁。

### 2026.8.29 / 5.10.268, 5.15.219, 6.1.186, 6.6.155, 6.12.107, 6.18.48, 7.1.12, 7.2.2

- 记录 2026-08-28 这批点版本，全部沿用原有补丁。上游没有改动 cjktty 触及的任何文件，所以没有补丁需要更新。

### 2026.8.28 / 5.10.267, 5.15.218, 6.1.185, 6.6.154, 6.12.106, 6.18.47, 7.1.11, 7.2.1

- 记录 2026-08-27 这批点版本，全部沿用原有补丁。上游只在 7.1.11 动到 `fbcon.c`，而且只移除两行 `EXPORT_SYMBOL`，其余 cjktty 触及的文件一个都没改，所以没有补丁需要更新。

### 2026.8.24 / 6.1.184, 5.15.217

- 增加上述两个点版本的补丁。同批其余五个版本沿用原有补丁，因为上游没有改动 cjktty 触及的任何文件。
- 上游在 2026-08-23 把 `struct fbcon_ops` 改名为 `struct fbcon_par` 回溯至 6.1 与 5.15，fbcon 前端因此拒绝这两份补丁。6.1 的补丁改用 6.12.104 的 fbcon 区段；5.15 的树结构相差过大，改为对被拒绝的 hunk 套用同一项改名。

### 2026.8.20 / 5.10.265, 5.15.216, 6.1.183, 6.6.152, 6.12.104, 6.18.45, 7.1.9

- 增加上述点版本的补丁。
- 上游在 `bit_cursor()` 中把游标字形索引限制在 `vc_font.charcount` 以内，七条分支因此拒绝该 hunk。`font_bits()` 新增边界检查，与 6.1、5.15、5.10 补丁中已有的一致。
...
- 将 `CONFIG_FONT_CJK_32x32` 设为默认关闭。以往补丁不带 32x32 字模，所以 `font_cjk_32x32.h` 为空，但这个选项自 2021 年加入以来一直默认开启，构建会把 8 MiB 的全零数据编入内核并报告成功。
